...
class FetchAndStoreProductsJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            // Log start of job
            Log::info("Starting FetchAndStoreProductsJob for page {$this->page}");
            ...
            if ($response->successful()) {
                ...
                Log::info("Fetched and processed products for page {$this->page}: " . $products->count() . " products");
                ...
                Log::error("Failed to fetch products for page {$this->page}: " . $response->status());
            }

            // Log end of job
            Log::info("Finished FetchAndStoreProductsJob for page {$this->page}");

        } catch (\Exception $e) {
            Log::error("Error in FetchAndStoreProductsJob: " . $e->getMessage());
            // Optionally, retry the job
            $this->release(60); // Delay for 60 seconds before retry
        }
    }
