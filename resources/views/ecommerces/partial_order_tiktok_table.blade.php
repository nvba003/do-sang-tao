@forelse ($orders as $order)
    <tr class="bg-white hover:bg-gray-100 border-b" data-order-id="{{ $order->id }}">
        ...
        <td class="w-4/24 p-2 text-xs sm:text-base overflow-hidden text-ellipsis">{{ $order->order_code }}</td>
        <td class="w-3/24 p-2 text-xs sm:text-base overflow-hidden text-ellipsis">{{ $order->customer_account }}</td>
        ...
        <td class="w-5/24 p-2 text-xs sm:text-base overflow-hidden text-ellipsis">{{ $order->order_date }}</td>
        ...
        <td id="order-id-{{ $order->id }}" class="w-2/24 p-2 text-xs sm:text-base overflow-hidden text-ellipsis">
            <span>{{ $order->order_id }}</span>
            @if ($order->order_id)
                <span class="text-green-500 ml-1">&#10004;</span>
            @endif
