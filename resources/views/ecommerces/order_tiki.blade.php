        ...
        <table id="orderTable" class="w-full bg-white border border-gray-200 rounded-lg" x-data="orderTable()">
            <thead class="text-white bg-gray-500">
                <tr>
                    ...
                    <th scope="col" class="w-3/24 px-2 py-3 text-left text-xs md:text-sm font-semibold uppercase tracking-wider">Khách hàng</th>
            ...
            <tbody>
                @include('ecommerces.partial_order_tiki_table', ['orders' => $orders, 'users' => $users, 'carriers' => $carriers])
            </tbody>
        </table>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        const baseUrl = `{{ url('/order-tiki') }}/${platformId}`;
        fetchData(baseUrl);
